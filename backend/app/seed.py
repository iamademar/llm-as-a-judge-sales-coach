"""
Demo data seeding for development and demo environments.

This module provides idempotent seeding of comprehensive demo data including:
- 3 organizations with varied SPIN performance profiles
...
- User accounts for each organization
...
def seed_organization(db: Session, org_name: str, org_description: str) -> Organization:
...
def seed_user_for_org(db: Session, organization: Organization, email: str) -> None:
    ...
    if existing:
        ...
        if existing.organization_id != organization.id:
            ...
            db.commit()
            logger.info(f"Updated user organization: {email}")
        else:
            logger.info(f"User already exists: {email} (password: {password})")
        return
    ...
    logger.info(f"Created user: {email} (password: {password})")
...
def seed_representatives(
    ...
    logger.info(f"Created/found {len(representatives)} representatives for {organization.name}")
...
def seed_transcripts_and_assessments(
    ...
    for rep in representatives:
        ...
        for start_date, end_date, month_index in months:
            # Check if transcripts already exist for this rep in this month
            existing_count = db.query(Transcript).filter(
                Transcript.representative_id == rep.id,
                Transcript.created_at >= start_date,
                Transcript.created_at <= end_date,
            ).count()

            if existing_count >= conversations_per_month:
                logger.info(f"Skipping {rep.full_name} for month {month_index + 10}/2025 - already has {existing_count} conversations")
                transcript_count += existing_count
                assessment_count += existing_count
                continue

            # Generate conversations for this month
            conversations_to_create = conversations_per_month - existing_count
            for _ in range(conversations_to_create):
...
def seed_demo_data():
    ...
    Creates:
    - 3 organizations with different performance profiles
    - 8 representatives per organization
    - ~25 conversations per rep per month for 3 months
    - Realistic assessments with score distributions and trending
    ...
    try:
        ...
        for org_name, profile in ORG_PROFILES.items():
            logger.info(f"Seeding organization: {org_name}")
            logger.info(f"  Profile: {profile['description']}")
            logger.info(f"  Composite range: {profile['composite_range']}")
            ...
            if not existing_templates:
                ...
                logger.info(f"  Created default prompt template")

            # Create representatives
            reps = seed_representatives(db, org, REP_NAMES[org_name])

            # Create transcripts and assessments
            logger.info(f"  Generating conversations (25 per rep per month for 3 months)...")
            ...
            logger.info(f"  Created {t_count} transcripts and {a_count} assessments for {org_name}")
        ...
        logger.info(f"  Organizations: {len(ORG_PROFILES)}")
        logger.info(f"  Representatives: {sum(len(names) for names in REP_NAMES.values())}")
        logger.info(f"  Transcripts: {total_transcripts}")
        logger.info(f"  Assessments: {total_assessments}")
        logger.info(f"")
        logger.info(f"User Login Credentials:")
        for org_name, profile in ORG_PROFILES.items():
            logger.info(f"  {profile['user_email']} / Password123!")
